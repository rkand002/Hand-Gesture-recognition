function predict_login= register(username)
...
if mode ==1
    ...
    cd ([cd ,'/',path]);
    save(['registerRawData_',username], 'sensorData');
    cd ../..
    ...
    cd ([cd ,'/',path]);
    save(['RegisterSet_',username], 'TestActions');
    cd ../..
    
   predict_login= testPredic_register(username);
